    ...
    def backward(ctx, doys: torch.Tensor, *args):
        ...
        for i in chunks[::-1]:
            ...
            if DEBUG:
                ...
                print(
                    ...
                    (
                        torch.autograd.grad(
                            _oys,
                            tmp_fwd_dtBus_div_rAts[chunks.index(i)],
                            doys,
                            create_graph=True,
                            allow_unused=True,
                        )[0]
            ...
            if DEBUG:
                ...
                print(
                    "4.0_v1",
                    (drAts_dtBus_div_rAts - drAts_dtBus_div_rAts_v1).abs().sum(),
                )
            ...
            if DEBUG:
                ...
                print(
                    "4.1",
                    (drAts_ref - (drAts_dtBus_div_rAts + drAts_hs_minus_prefix)).abs().sum(),
                )
                print(
                    "4.2",
                    ((drAts_ref - (drAts_dtBus_div_rAts + drAts_hs_minus_prefix)) / scale).abs().sum(),
                )
...
def selective_scan_easyv2(
    ...
    def ss_chunk(us, dts, As, Bs, Cs, h0, mask):
        ...
        y_1 = torch.einsum(
            "bhlrv,bhrv->bhlv",
            torch.einsum("bhlkv,bhrkv->bhlrv", q_mul_w, k_div_w) * _mask,
            v,
        )
...
def selective_scan_easyv3(
    ...
    ht1, y1, ws, cws, q_mul_w = ss_chunk_h1y1(Cs, Bs, _us * dts, None, As, None, dts, mask=_mask_tril)
...
def selective_scan_ref(
    ...
    is_variable_B = B.embed_dim() >= 3
    is_variable_C = C.embed_dim() >= 3
    if A.is_complex():
        if is_variable_B:
