...
class PatchMerging2D(nn.Module):
    def __init__(self, dim, out_dim=-1, norm_layer=nn.LayerNorm, channel_first=False):
        ...
        self._patch_merging_pad = (
            self._patch_merging_pad_channel_first if channel_first else self._patch_merging_pad_channel_last
        )
...
class SS2Dv0:
    def __initv0__(
        ...
        **kwargs,
        ...
        # dt proj ============================
        self.dt_projs = [
            self.dt_init(dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor) for _ in range(k_group)
        ]
        ...
        if cascade2d:
            ...
            y_col = (
                scan_rowcol(
                    ...
                    dt_bias=(
                        dt_projs_bias.view(4, -1)[2:].contiguous().to(y_row.dtype)
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        us, dts, Bs, Cs = x.split(
            [self.d_inner, self.dts_dim, 4 * self.d_state, 4 * self.d_state],
        ...
        if self.channel_first:
            ...
            if self.dts_dim == self.dt_rank:
                ...
                dts = F.conv1d(
                    dts,
                    dt_projs_weight.view(4 * self.d_inner, self.dt_rank, 1),
                    None,
                    groups=4,
                )
        ...
        ys: torch.Tensor = selective_scan(us, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus).view(B, 4, -1, H, W)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        ss2d_layer: Type[nn.Module] = SS2D,
        **kwargs,
    ):
        super().__init__()
        ...
        if self.mlp_branch:
            ...
            _MLP = kwargs.get("mlp_layer") or _MLP
            assert _MLP is not None, "invalid mlp config"
