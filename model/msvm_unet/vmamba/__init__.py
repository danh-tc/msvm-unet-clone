...
def load_pretrained_ckpt(model: VSSM, ckpt_fn: str) -> VSSM:
    ...
    logger.info(f"Loaded weights from: {ckpt_fn}")
...
def build_model(config: Dict[str, Any], ckpt: Optional[str] = None, **kwargs: Any) -> VSSM:
    ...
    if ckpt is not None and osp.exists(ckpt):
        model = load_pretrained_ckpt(model=model, ckpt_fn=ckpt)
    return model


def build_tiny_0230s(**kwargs: Any) -> VSSM:
    if kwargs.pop("pretrained", False):
        kwargs["ckpt"] = osp.expandvars(osp.join("${PRETRAIN_HOME}", "vssm1_tiny_0230s_ckpt_epoch_264.pth"))
...
def build_small_0229s(**kwargs: Any) -> VSSM:
    if kwargs.pop("pretrained", False):
        kwargs["ckpt"] = osp.expandvars(osp.join("${PRETRAIN_HOME}", "vssm1_small_0229s_ckpt_epoch_240.pth"))
...
def build_base_0229s(**kwargs: Any) -> VSSM:
    if kwargs.pop("pretrained", False):
        kwargs["ckpt"] = osp.expandvars(osp.join("${PRETRAIN_HOME}", "vssm1_base_0229s_ckpt_epoch_225.pth"))
...
ENCODERS = {
    "tiny": build_tiny_0230s,
    "small": build_small_0229s,
    "base": build_base_0229s,
}
