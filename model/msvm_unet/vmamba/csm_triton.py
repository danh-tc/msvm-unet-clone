...
class CrossScanTritonF(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](
            x, y, (0 if channel_first else 1), 0, 0, 0, BC, BH, BW, C, H, W, NH, NW
        )
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](
            x, y, (0 if channel_first else 1), 0, 1, 0, BC, BH, BW, C, H, W, NH, NW
        )
        return x, None
...
class CrossScanTriton1b1F(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, channel_first=True):
        ...
        y = x.new_empty((B, 4, C, H, W))
        triton_cross_scan_flex[(NH * NW, NC, B)](
            x, y, (0 if channel_first else 1), 0, 0, 1, BC, BH, BW, C, H, W, NH, NW
        )
        return y.view(B, 4, C, -1)
    ...
    def backward(ctx, y: torch.Tensor):
        ...
        triton_cross_scan_flex[(NH * NW, NC, B)](
            x, y, (0 if channel_first else 1), 0, 1, 1, BC, BH, BW, C, H, W, NH, NW
        )
        return x, None
        ...
        def cross_scan_1b1(x: torch.Tensor):
            ...
            xs = torch.stack(
                [
                    ...
                    torch.flip(
                        torch.transpose(x[:, 3], dim0=2, dim1=3).contiguous().view(B, C, L),
                        dims=[-1],
                    ),
                ],
                dim=1,
