...
from .vmamba.vmamba import LayerNorm2d, Linear2d, VSSBlock
            ...
            self.out_layers = nn.ModuleList(
                [
                    nn.Sequential(
                        ...
                        nn.UpsamplingBilinear2d(scale_factor=2 ** (len(dims) - i)),
                    )
                    for i, ch in enumerate(dims[1:])
                ]
            )
        else:
    ...
    def forward(self, features: Sequence[Tensor]) -> Tensor:
        ...
        if self.deep_supervision:
            ...
            for i, layer in enumerate(self.layers):
                ...
                preds.append(self.out_layers[i](out))
