"""Copied from https://github.com/JiarunLiu/Swin-UMamba.
"""
...
class SwinUMamba(nn.Module):
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(in_channels, feat_size[0], kernel_size=7, stride=2, padding=3),
...
def load_pretrained_ckpt(model):
    ckpt_path = osp.expandvars(osp.join("$PRETRAIN_HOME", "vmamba_tiny_e292.pth"))
    ...
    for k, v in ckpt["model"].items():
        if k in skip_params:
            logger.info(f"Skipping weights: {k}")
            continue
        kr = f"vssm_encoder.{k}"
        if "downsample" in kr:
        ...
        if kr in model_dict.keys():
            ...
            logger.info(f"Passing weights: {k}")
